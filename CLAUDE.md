...
Single user. Single source. No hardware to own or maintain — the collector
runs as a scheduled **Cloud Run Job**, not a Pi or a VPS. Optimizing for
*reliability over months*, not scale.

Stack: TypeScript · Node 20 · Playwright · Firebase (Firestore) · Google Cloud
Run Jobs + Cloud Scheduler · Anthropic API · Telegram Bot API.
...
npm run once               # one full collection cycle, no scheduler — this is what the Cloud Run Job invokes
npm run collect            # local-only dev loop; production scheduling is Cloud Scheduler, not this process
npm run emulate             # firebase emulators (firestore)
npm run deploy:job          # build + push the collector container, deploy the Cloud Run Job, sync the Cloud Scheduler trigger
...
5. **Anti-bot challenges are a stop signal.**
   ...
   spoofing, or proxy rotation.** Running the collector on Cloud Run Jobs
   already trades a residential-shaped IP for a Google datacenter one — that
   *raises* challenge risk, it doesn't lower it. If a static outbound IP (via
   Cloud NAT) is added, it is for *consistency* — the same IP returning every
   cycle, like a repeat visitor — never for evasion. If challenges recur, the
   fix is to slow the poll interval further or ask TanitJobs for feed access.
   Beyond the ToS problem, grinding against a WAF turns a soft block into a
...
- **No public webhook for v1.** Outbound-only delivery needs nothing
  publicly reachable — the Cloud Run Job calls the Bot API directly. A
  webhook (or long-polling) only becomes necessary for inbound commands
...
- One browser *session*, reused across ephemeral runs. Cloud Run Jobs have no
  persistent disk, so Playwright `storageState` (cookies + localStorage) round-
  trips through a small Cloud Storage object at the start and end of every run
  — the same "returning visitor" shape as a persistent profile, just
  serialized between executions instead of left on disk.
- Jitter both the inter-request delay and the cycle interval. Default the
  Cloud Scheduler cadence to **every 15 minutes**, not 5 — a cold start plus a
  Google-datacenter egress IP is already a heavier signal than a home
  connection was; there is no reason to also make it a frequent one.
- Never poll faster than every 5 minutes, full stop.

---

## Hosting: no hardware to own

The collector runs as a **Cloud Run Job** (run-to-completion, not a Cloud Run
*Service*), invoked on a schedule by **Cloud Scheduler**, in the same GCP
project as Firestore. It boots, does one cycle (sweep → dedup → detail fetch →
classify → score → notify), and exits. Nothing stays running between cycles;
nothing needs a systemd unit, SSH access, or OS patching.

The trade against a self-hosted box: a Google datacenter egress IP instead of
a residential one, and no persistent disk (mitigated by `storageState` in
Cloud Storage, see above). The trade in favor: zero physical or virtual
hardware to maintain, ever. If a change seems to need "the machine it runs
on" to persist state in memory across cycles, that's a sign the state belongs
in Firestore instead, not a sign to bring back an always-on box.

If a public HTTPS endpoint is ever genuinely needed (inbound Telegram
commands), it is a small, separate Cloud Function or Cloud Run Service — never
